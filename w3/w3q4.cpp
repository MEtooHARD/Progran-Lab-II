...
class Integer {
 ...
 public:
   ...
   * Integer */
  Integer addition(Integer b) const {
  ...
  Digit digit(int digit) const { return value[digit]; }
  ...
  /* returns a reversed Integer based on this Integer */
  Integer reverse() const {
    Integer reversed;
    for (int i = digits - 1; i >= 0; i--)
      reversed.appendDigit(digit(i).getValue());
    return reversed;
  }

  bool isPlindrome() {
    for (int i = 0; i < digits / 2; i++)
      if (digit(i).getValue() != digit(digits - 1 - i).getValue()) return false;
    return true;
  }
};
int main() {
  ...
  for (int i = 0; i < amount; i++) {
    ...
    do {
      count++;
      num = num.addition(num.reverse());
    } while (!num.isPlindrome());
    cout << count << ' ';
    num.display();
    cout << endl;
  }
